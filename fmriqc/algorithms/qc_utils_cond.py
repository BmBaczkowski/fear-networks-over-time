...
def plot_power(FD_file, 
    ...
    sns.tsplot(FD, range(1,414), color='k', ax=ax1, linewidth=0.8)
    [ax1.axvspan(x, x+1, facecolor='b', alpha=.5, fill=True) for x in CSminus]
    [ax1.axvspan(x, x+1, facecolor='g', alpha=.5, fill=True) for x in CSplus]
    [ax1.axvspan(x, x+1, facecolor='r', alpha=.5, fill=True) for x in shock]
    p1 = mpatches.Patch(color='b', label='CS-')
    p2 = mpatches.Patch(color='g', label='CS+')
    p3 = mpatches.Patch(color='r', label='UCS')
    ax1.legend(handles=[p1,p2,p3], bbox_to_anchor=(1, 1), loc=2)
    ax1.set_xticks([])
    ...
    sns.tsplot(DVARSpre, range(2,414), color='b', ax=ax2, linewidth=0.8)
    sns.tsplot(DVARSpost-1, range(2,414), color='r', ax=ax2, linewidth=0.8)
    ...
    sns.tsplot(GS_pre, range(1,414), color='g', ax=ax3, linewidth=0.8)
    sns.tsplot(GS_post, range(1,414), color='y', ax=ax3, linewidth=0.8)
    sns.tsplot(SCR-np.max(SCR)-.8, onsets, color='b', ax=ax3, marker='o', linewidth=0.5, markersize=2)
    #ax3.set_ylabel('BOLD')
